...
from smpl_sim.utils.mujoco_utils import get_body_qposaddr
...
def run(in_file: str, out_file: str):
    ...
    for key_name in tqdm(amass_data.keys()):
        ...
        root_trans_raw = smpl_data_entry["trans"].copy()[start:]
        B = pose_aa.shape[0]
        ...
        root_trans = root_trans_raw.copy()
        root_trans[:, 1] = -root_trans_raw[:, 2]
        root_trans[:, 2] = root_trans_raw[:, 1]
        beta = (
...
if __name__ == "__main__":
    # file_ids = ["007", "008", "009", "010", "011", "012", "013", "014", "015", "016"]
    # for file_id in file_ids:
    #     in_file = os.path.join("sample_data", f"dao_{file_id}_raw.pkl")
    #     out_file = os.path.join("sample_data", f"dao_{file_id}_isaac.pkl")
    #     run(in_file=in_file, out_file=out_file)
    # exit()
    in_file = os.path.join("sample_data", f"dao_full_front_raw.pkl")
    out_file = os.path.join("sample_data", f"dao_full_front_isaac.pkl")
    run(in_file=in_file, out_file=out_file)
    exit()
